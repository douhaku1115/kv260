# ============================================================
# RISC-V on KV260 - 2ワードブロック・キャッシュ版 (4段 m_proc8_c2b + m_cache2 + バースト主記憶) VIO project
#   命令メモリは逐次プログラム(asm_seq.txt, addi×50, 結果50)。
# Usage: vivado -mode batch -source create_block.tcl
# ============================================================

set project_name "riscv_kv260_block"
...
# 設計ファイル(ブロックキャッシュ版) + プログラム(逐次)
add_files -norecurse ${src_dir}/main_vio_block.v
add_files -norecurse ${src_dir}/asm_seq.txt
set_property file_type "Verilog Header" [get_files asm_seq.txt]
...
puts "BIT: $bit"
puts "LTX: $ltx"
puts "BUILD OK (block: 4stage m_proc8_c2b + m_cache2 + burst imem)"
