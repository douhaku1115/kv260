...
#   gcc生成の asm_rvc.txt(既定=Σ0..100=5050) を命令メモリに焼く。
#   結果は VIO で読む: probe0=w_rslt(結果=0x13BA), probe1=w_done(1)。
#   外部ピン無し(VIOのみ)。
#   ※ 先に src/gcc/build_gcc.sh を実行して src/asm_rvc.txt を生成しておくこと。
# Usage: vivado -mode batch -source create_rv32i.tcl
# ============================================================

set project_name "riscv_kv260_rvc"
...
add_files -norecurse ${src_dir}/main_vio_rvc.v
add_files -norecurse ${src_dir}/asm_rvc.txt
set_property file_type "Verilog Header" [get_files asm_rvc.txt]
add_files -fileset constrs_1 -norecurse ${src_dir}/uart_console_pins.xdc
...
puts "BIT: $bit"
puts "LTX: $ltx"
puts "BUILD OK (kozos: KOZOS interactive shell + kernel (ps/run/tick) -> VIO)"
