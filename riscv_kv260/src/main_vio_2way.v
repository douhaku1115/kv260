// ============================================================
// RISC-V on KV260 - 2-wayキャッシュ版 (4段 m_proc8_c2 + 2-way m_cache3 + 遅延主記憶)
//   教科書8章: メモリ階層(セットアソシアティブ)。
//     m_imem    (code8-2)  : D_DELAYサイクル遅延の主記憶(プログラムを焼込み)
//     m_cache3  (code8-12) : 2-wayセットアソシアティブ命令キャッシュ(LRU付き)
//     m_proc8_c2(code8-8)  : 4段プロセッサ+キャッシュ。ヒット=即/ミス=主記憶待ちでストール
//
//   ※ 2-wayが直接マップに勝つのは「コンフリクトミス」がある時。
//     プログラムは asm_conflict.txt (code8-11): 低位0x0cと高位0x8c等が同じインデックスに当たり、
//     直接マップでは毎周追い出し合う(スラッシング)→2-wayなら共存できる。
//   効果は「総サイクル数」で観測(結果5050は不変)。
...
module m_imem(w_clk, w_pc, w_re, r_insn, r_oe);
  ...
  initial begin
    `define MM mem
    `include "asm_conflict.txt"   // コンフリクト用(code8-11, 結果5050)
  end
endmodule
...
// ---- 2-wayセットアソシアティブ命令キャッシュ (教科書 code8-12) ----
//   直接マップ m_cache1 を2way並べ、LRU(1bit)で追い出すwayを選ぶ。
//   同じインデックスに当たる2アドレスが共存でき、コンフリクトミスを防ぐ。
