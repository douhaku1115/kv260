...
// 5段パイプライン 完全RV32I
// ============================================================
module m_proc_rvsys(w_clk, r_rslt, r_done);
  input  wire w_clk;
  ...
  // ---- リダイレクト(P2境界): 分岐/ジャンプ + トラップ(ecall/mret) ----
  wire w_brcond;
  ...
  wire w_redir     = w_take_b | w_take_jal | w_take_jalr | w_trap_e | w_trap_r;
  assign w_pcin = (w_trap_e)               ? csr_mtvec :          // ecall: ->mtvec
  ...
  assign w_ldd = (P3_f3==3'b000) ? {{24{w_sh[7]}},  w_sh[7:0]}  :   // lb
...
// ---- KV260 top: PSのpl_clk0 + m_proc_rvsys + VIO(結果/done) ----
module m_top_kv260;
  wire w_clk;
  wire [31:0] w_rslt, w_done;
  clk_bd_wrapper m0 (.pl_clk0(w_clk));
  m_proc_rvsys m1 (w_clk, w_rslt, w_done);
