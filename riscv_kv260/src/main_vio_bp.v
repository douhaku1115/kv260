// ============================================================
// RISC-V on KV260 - 分岐予測版 (5段 m_proc9 + BTB + bimodal 2bit)
//   ベース: main_vio_4stage.v の m_proc9 (5段パイプライン)
//   追加:
//     m_btb     (教科書 code7-4)  : 分岐先キャッシュ
//     m_bimodal (教科書 code7-11) : 2bit飽和カウンタ予測器
//   PC選択を教科書 code7-9 (BTB+PHT) の bimodal 版に差し替え。
//
//   ※ 分岐予測を入れても計算結果は 5050 のまま変わらない。
//     効果は「ミス予測回数」で観測する:
//       予測なし : 取られる分岐ごとに毎回ミス → 約100回
//       BTB+bimodal : ウォームアップ+ループ脱出のみ → 数回
//   VIOに3本出力: w_rslt(結果0x13BA) / w_miss(ミス回数) / w_brn(分岐回数)
// ============================================================
...
// ---- bimodal 2bit飽和カウンタ予測器 (教科書 code7-11) ----
...
module m_proc9(w_clk, r_dout, r_miss, r_brn);
  ...
  // ---- 分岐予測: BTB + bimodal(2bit) ----
  wire [31:0] w_ppc;             // BTBが返す予測分岐先
  wire w_pred;                   // bimodalの予測 (taken?)
  wire w_hit;                    // BTBヒット
  m_bimodal m15 (w_clk, r_pc[6:2], w_pred,
