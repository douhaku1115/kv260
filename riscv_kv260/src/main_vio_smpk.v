...
// 5段パイプライン 完全RV32I
// ============================================================
module m_kcore(w_clk, w_hartid, r_rslt, r_done,
               w_ex_adr, w_ex_wd, w_ex_we, w_ex_rdreq, w_ex_rd,
  ...
  // 外部共有バス(0x0004=共有UART, 0x0006=共有ブロック lock/IPI/done)
  output wire [31:0] w_ex_adr, w_ex_wd;
  output wire w_ex_we, w_ex_rdreq;
  input  wire [31:0] w_ex_rd;
  // 共有RAM(0x0007_xxxx, スレッドTCB/スタック)
  output wire [31:0] w_sr_adr, w_sr_wd;
  output wire w_sr_we;  output wire [3:0] w_sr_be;
  input  wire [31:0] w_sr_rd;
  input  wire w_ipi_pend;          // このコア宛のIPI保留
  input  wire w_rx_irq;            // UART RX割込み(共有UARTから, core0のみ配線)
  ...
  wire [31:0] w_csr_old =
       ...
       (P2_csraddr==12'h344) ? {20'd0, w_meip, 3'd0, w_mtip, 3'd0, w_ipi_pend, 3'd0} :  // mip: 11=MEIP,7=MTIP,3=MSIP
                               32'd0;
  ...
  wire w_meip      = w_rx_irq;                       // 外部割込み保留(共有UART RX, core0のみ)
  wire w_ext_pend  = csr_mie[11] & w_meip;           // 外部(UART)割込み
  wire w_tmr_pend  = csr_mie[7]  & w_mtip;           // タイマ割込み
  wire w_ipi_p     = csr_mie[3]  & w_ipi_pend;       // IPI(ソフトウェア割込み)
  wire w_irq       = csr_mstatus[3] & (w_ext_pend | w_tmr_pend | w_ipi_p);
  ...
  wire [31:0] w_ldd_mem =
                 ...
                 (P3_f3==3'b101) ? {16'd0,          w_sh[15:0]} :   // lhu
                                    w_ld_word;                      // lw
  // ---- 外部共有バス(0x0004=共有UART[番地は従来のまま], 0x0006=共有ブロック lock/IPI/done) ----
  wire w_is_uart   = (P3_alu[31:16]==16'h0004);
  wire w_is_shblk  = (P3_alu[31:16]==16'h0006);
  wire w_is_hartid = (P3_alu==32'h0006_0008);             // hartid(コア内で返す)
  wire w_is_ext    = w_is_uart | w_is_shblk;
  assign w_ex_adr   = P3_alu;
  assign w_ex_wd    = P3_in3;
  assign w_ex_we    = w_st & w_is_ext;
  assign w_ex_rdreq = P3_ld & P3_v & w_is_ext;            // TAS/RX-popの副作用に必要
  // ---- 共有RAM(0x0007_xxxx: スレッドTCB/スタック) ----
  wire w_is_sram = (P3_alu[31:16]==16'h0007);
  ...
  assign w_ldd = w_is_hartid ? {30'd0, w_hartid} :
                 w_is_ext    ? w_ex_rd           :   // 共有UART/共有ブロック
                 w_is_timer  ? w_timer_rd        :
                 w_is_bp     ? w_bp_rd           : w_ldd_mem;
  ...
  always @(posedge w_clk) if (!w_lduse) begin
    if (w_take_irq) begin                // 割込みエントリ(P2命令に相乗り→復帰点)
      ...
      csr_mcause    <= w_ext_pend ? 32'h8000_000B :               // 外部(UART)=11
                       w_tmr_pend ? 32'h8000_0007 : 32'h8000_0003; // タイマ=7 / IPI=3
...
// ============================================================
// 共有UART(0x0004_xxxx, 2ポート調停): TX=0x40000(w) STAT=0x40004(r) RX=0x40008(r,pop)
//   TX同時書込はcore0優先(ソフトはロックで直列化する規律)。RX popもcore0優先(シェルはcore0のみ)。
// ============================================================
module m_uart_shared(w_clk, uart_tx, uart_rx, w_rx_irq,
  a0, wd0, we0, rq0, rd0,  a1, wd1, we1, rq1, rd1);
  input  wire w_clk, uart_rx;
  output wire uart_tx, w_rx_irq;
  input  wire [31:0] a0, wd0, a1, wd1;
  input  wire we0, rq0, we1, rq1;
  output wire [31:0] rd0, rd1;
  wire w_tx_full, w_rx_valid; wire [7:0] w_rx_byte;
  wire tx0 = we0 & (a0==32'h0004_0000);
  wire tx1 = we1 & (a1==32'h0004_0000);
  wire w_tx_we = tx0 | tx1;                              // core0優先(同時はcore1破棄=SWロックで回避)
  wire [7:0] w_tx_d = tx0 ? wd0[7:0] : wd1[7:0];
  wire rx0 = rq0 & (a0==32'h0004_0008);
  wire rx1 = rq1 & (a1==32'h0004_0008);
  m_uart_tx #(.DIV(`UARTDIV)) u_tx (w_clk, w_tx_we, w_tx_d, uart_tx, w_tx_full);
  m_uart_rx #(.DIV(`UARTDIV)) u_rx (w_clk, uart_rx, rx0|rx1, w_rx_byte, w_rx_valid);
  wire [31:0] w_stat = {30'd0, w_tx_full, w_rx_valid};
  assign rd0 = (a0==32'h0004_0004) ? w_stat : (a0==32'h0004_0008) ? {24'd0,w_rx_byte} : 32'd0;
  assign rd1 = (a1==32'h0004_0004) ? w_stat : (a1==32'h0004_0008) ? {24'd0,w_rx_byte} : 32'd0;
  assign w_rx_irq = w_rx_valid;
endmodule

// ============================================================
// 共有ブロック(0x0006_xxxx): TASロック + IPI + done + 汎用共有レジスタ8本
//   0x60000=lock(読=TAS/書=解放, 同時TASはcore0優先) 0x60004=カウンタ 0x6000C=done(OR)
//   0x60010=IPI送信(bitで対象) 0x60014=IPI ack  0x60020-3C=汎用r_rf[0:7]
// ============================================================
module m_kshared(w_clk, a0,wd0,we0,rq0,rd0, a1,wd1,we1,rq1,rd1,
                 r_counter, r_done, r_ipi0, r_ipi1);
  input  wire w_clk;
  input  wire [31:0] a0,wd0,a1,wd1;
  input  wire we0,rq0,we1,rq1;
  output wire [31:0] rd0, rd1;
  output reg [31:0] r_counter=0, r_done=0;
  output reg r_ipi0=0, r_ipi1=0;
  reg r_lock=0;
  reg [31:0] r_rf [0:7];
  integer k; initial for(k=0;k<8;k=k+1) r_rf[k]=0;
  wire lk0=(a0==32'h0006_0000), ct0=(a0==32'h0006_0004), dn0=(a0==32'h0006_000C);
  wire lk1=(a1==32'h0006_0000), ct1=(a1==32'h0006_0004), dn1=(a1==32'h0006_000C);
  wire rf0=(a0[31:5]==27'h3001), rf1=(a1[31:5]==27'h3001);  // 0x60020-3C
  wire [2:0] ix0=a0[4:2], ix1=a1[4:2];
  wire tas0=rq0&lk0, tas1=rq1&lk1, rel0=we0&lk0, rel1=we1&lk1;
  wire acq0=tas0&~r_lock, acq1=tas1&~r_lock&~tas0;
  assign rd0 = lk0 ? {31'd0,r_lock}      : ct0 ? r_counter : rf0 ? r_rf[ix0] : 32'd0;
  assign rd1 = lk1 ? {31'd0,r_lock|acq0} : ct1 ? r_counter : rf1 ? r_rf[ix1] : 32'd0;
  wire sn0=we0&(a0==32'h0006_0010), sn1=we1&(a1==32'h0006_0010);
  wire ak0=we0&(a0==32'h0006_0014), ak1=we1&(a1==32'h0006_0014);
  wire set0=(sn0&wd0[0])|(sn1&wd1[0]), set1=(sn0&wd0[1])|(sn1&wd1[1]);
  always @(posedge w_clk) begin
    if (rel0|rel1) r_lock<=0; else if (acq0|acq1) r_lock<=1;
    if (we0&ct0) r_counter<=wd0; else if (we1&ct1) r_counter<=wd1;
    r_done <= r_done | (we0&dn0 ? wd0:0) | (we1&dn1 ? wd1:0);
    r_ipi0 <= set0 ? 1'b1 : (ak0 ? 1'b0 : r_ipi0);
    r_ipi1 <= set1 ? 1'b1 : (ak1 ? 1'b0 : r_ipi1);
    if (we0&rf0) r_rf[ix0]<=wd0;
    if (we1&rf1 & ~(we0&rf0&(ix0==ix1))) r_rf[ix1]<=wd1;
  end
endmodule
...
module m_top_kv260(uart_tx, uart_rx);
  ...
  input  wire uart_rx;
  wire w_clk;
  wire [31:0] w_rslt0, w_done0, w_rslt1, w_done1;
  wire [31:0] ea0,ed0,er0, ea1,ed1,er1;         // 外部共有バス
  wire ewe0,erq0, ewe1,erq1;
  wire [31:0] sa0,sd0,srd0, sa1,sd1,srd1;       // 共有RAMバス
  wire swe0,swe1; wire [3:0] sbe0,sbe1;
  wire ipi0, ipi1, rx_irq;
  wire [31:0] u_rd0, u_rd1, b_rd0, b_rd1;
  wire [31:0] w_cnt, w_dn;
  clk_bd_wrapper m0 (.pl_clk0(w_clk));
  m_kcore c0 (w_clk, 2'd0, w_rslt0, w_done0, ea0,ed0,ewe0,erq0,er0,
              sa0,sd0,swe0,sbe0,srd0, ipi0, rx_irq);
  m_kcore c1 (w_clk, 2'd1, w_rslt1, w_done1, ea1,ed1,ewe1,erq1,er1,
              sa1,sd1,swe1,sbe1,srd1, ipi1, 1'b0);   // RX割込みはcore0のみ
  // 共有UART(0x0004)と共有ブロック(0x0006)は同じ外部バスに載る: 読みデータをOR合成
  m_uart_shared uu (w_clk, uart_tx, uart_rx, rx_irq,
                    ea0,ed0,ewe0,erq0,u_rd0, ea1,ed1,ewe1,erq1,u_rd1);
  m_kshared     bb (w_clk, ea0,ed0,ewe0,erq0,b_rd0, ea1,ed1,ewe1,erq1,b_rd1,
                    w_cnt, w_dn, ipi0, ipi1);
  assign er0 = u_rd0 | b_rd0;   // アドレスで排他デコードなのでORでmux相当
  assign er1 = u_rd1 | b_rd1;
  m_ksram sr (w_clk, sa0,swe0,sbe0,sd0,srd0, sa1,swe1,sbe1,sd1,srd1);
  vio_0 m3 (w_clk, w_rslt0, w_cnt);   // VIO: core0結果 / 共有カウンタ
endmodule
