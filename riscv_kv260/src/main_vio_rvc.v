...
module m_am_imem(w_pc, w_insn, w_insn_hi, w_dadr, w_ddata);
  input  wire [31:0] w_pc, w_dadr;
  output wire [31:0] w_insn, w_insn_hi, w_ddata;
  reg [31:0] mem [0:4095];
  assign w_insn    = mem[w_pc[13:2]];
  assign w_insn_hi = mem[w_pc[13:2]+12'd1]; // 次ワード(RVC: 境界またぎ32bit命令の上位半分)
  assign w_ddata = mem[w_dadr[13:2]];       // データ読み出しポート(rodata/文字列リテラル用)
  integer i; initial for (i=0; i<4096; i=i+1) mem[i] = 32'd0;
`ifndef PROG
  `define PROG "asm_rvc.txt"
`endif
  initial begin
    `define MM mem
    `include `PROG
...
module m_btb(w_clk, w_rpc, w_hit, w_tgt, w_wpc, w_we, w_wtgt);
  ...
  reg [57:0] mem [0:63];                    // {valid(1), tag={pc[31:8],pc[1]}(25), target(32)}
  // RVC: 2バイト違いのPCが同一indexに乗るため、タグにpc[1]を含めて誤ヒットを防ぐ(正当性に必須)
  always @(posedge w_clk) if (w_we) mem[w_wpc[7:2]] <= {1'b1, w_wpc[31:8], w_wpc[1], w_wtgt};
  wire w_v; wire [24:0] w_tag; wire [31:0] w_data;
  assign {w_v, w_tag, w_data} = mem[w_rpc[7:2]];
  assign w_hit = w_v & (w_tag == {w_rpc[31:8], w_rpc[1]});
  assign w_tgt = w_data;
...
// ============================================================
// RV32C 展開器: 16bit圧縮命令 → 等価な32bit命令 (組合せ)
//   quadrant0/1/2 の RV32 サブセット(F/D系は除く)。未知は nop に展開。
// ============================================================
module m_rvc(w_ci, w_ir);
  input  wire [15:0] w_ci;
  output reg  [31:0] w_ir;
  // レジスタフィールド
  wire [4:0] rd   = w_ci[11:7];             // フル(rd/rs1)
  wire [4:0] rs2  = w_ci[6:2];              // フル rs2
  wire [4:0] rdp  = {2'b01, w_ci[4:2]};     // rd'/rs2' (x8-x15)
  wire [4:0] rs1p = {2'b01, w_ci[9:7]};     // rs1'/rd'
  wire s = w_ci[12];                        // 符号ビット(多くの形式で)
  // 即値(形式ごとのスクランブル解き)
  wire [11:0] i_4spn = {2'b00, w_ci[10:7], w_ci[12:11], w_ci[5], w_ci[6], 2'b00}; // c.addi4spn(ゼロ拡張)
  wire [11:0] i_lw   = {5'b00000, w_ci[5], w_ci[12:10], w_ci[6], 2'b00};          // c.lw/c.sw
  wire [11:0] i_imm6 = {{7{s}}, w_ci[6:2]};                                       // c.addi/c.li/c.andi(符号)
  wire [11:0] i_16sp = {{3{s}}, w_ci[4:3], w_ci[5], w_ci[2], w_ci[6], 4'b0000};   // c.addi16sp(符号,16倍)
  wire [19:0] i_lui  = {{15{s}}, w_ci[6:2]};                                      // c.lui(符号,<<12前)
  wire [11:0] i_lwsp = {4'b0000, w_ci[3:2], w_ci[12], w_ci[6:4], 2'b00};          // c.lwsp
  wire [11:0] i_swsp = {4'b0000, w_ci[8:7], w_ci[12:9], 2'b00};                   // c.swsp
  wire [4:0]  shamt  = w_ci[6:2];                                                 // c.slli/srli/srai(RV32)
  // c.j/c.jal オフセット(12bit符号,×2) → jal即値フィールド{imm20,imm10:1,imm11,imm19:12}
  wire [10:1] jt = {w_ci[8], w_ci[10:9], w_ci[6], w_ci[7], w_ci[2], w_ci[11], w_ci[5:3]};
  wire [19:0] j_fld = {s, jt, s, {8{s}}};
  // c.beqz/c.bnez オフセット(9bit符号,×2) → B形式フィールド
  wire [10:5] bt_h = {{3{s}}, w_ci[6:5], w_ci[2]};               // imm[10:5]={s,s,s,ci6,ci5,ci2}
  wire [4:1]  bt_l = {w_ci[11:10], w_ci[4:3]};                   // imm[4:1]
  always @(*) begin
    w_ir = 32'h13;                                               // 既定=nop(未知圧縮命令)
    case ({w_ci[1:0], w_ci[15:13]})
      // ---- quadrant 0 ----
      5'b00_000: if(i_4spn!=0) w_ir = {i_4spn, 5'd2, 3'b000, rdp, 7'h13};      // c.addi4spn
      5'b00_010: w_ir = {i_lw, rs1p, 3'b010, rdp, 7'h03};                      // c.lw
      5'b00_110: w_ir = {i_lw[11:5], rdp, rs1p, 3'b010, i_lw[4:0], 7'h23};     // c.sw
      // ---- quadrant 1 ----
      5'b01_000: w_ir = {i_imm6, rd, 3'b000, rd, 7'h13};                       // c.addi/c.nop
      5'b01_001: w_ir = {j_fld, 5'd1, 7'h6F};                                  // c.jal(RV32)
      5'b01_010: w_ir = {i_imm6, 5'd0, 3'b000, rd, 7'h13};                     // c.li
      5'b01_011: w_ir = (rd==5'd2) ? {i_16sp, 5'd2, 3'b000, 5'd2, 7'h13}       // c.addi16sp
                                   : {i_lui, rd, 7'h37};                       // c.lui
      5'b01_100: case (w_ci[11:10])
          2'b00: w_ir = {7'h00, shamt, rs1p, 3'b101, rs1p, 7'h13};             // c.srli
          2'b01: w_ir = {7'h20, shamt, rs1p, 3'b101, rs1p, 7'h13};             // c.srai
          2'b10: w_ir = {i_imm6, rs1p, 3'b111, rs1p, 7'h13};                   // c.andi
          2'b11: case (w_ci[6:5])
              2'b00: w_ir = {7'h20, rdp, rs1p, 3'b000, rs1p, 7'h33};           // c.sub
              2'b01: w_ir = {7'h00, rdp, rs1p, 3'b100, rs1p, 7'h33};           // c.xor
              2'b10: w_ir = {7'h00, rdp, rs1p, 3'b110, rs1p, 7'h33};           // c.or
              2'b11: w_ir = {7'h00, rdp, rs1p, 3'b111, rs1p, 7'h33};           // c.and
          endcase
      endcase
      5'b01_101: w_ir = {j_fld, 5'd0, 7'h6F};                                  // c.j
      5'b01_110: w_ir = {s, bt_h, 5'd0, rs1p, 3'b000, bt_l, s, 7'h63};         // c.beqz
      5'b01_111: w_ir = {s, bt_h, 5'd0, rs1p, 3'b001, bt_l, s, 7'h63};         // c.bnez
      // ---- quadrant 2 ----
      5'b10_000: w_ir = {7'h00, shamt, rd, 3'b001, rd, 7'h13};                 // c.slli
      5'b10_010: w_ir = {i_lwsp, 5'd2, 3'b010, rd, 7'h03};                     // c.lwsp
      5'b10_100: w_ir = (w_ci[12]==1'b0)
                 ? ((rs2==5'd0) ? {12'h0, rd, 3'b000, 5'd0, 7'h67}             // c.jr
                                : {7'h00, rs2, 5'd0, 3'b000, rd, 7'h33})       // c.mv
                 : ((rs2==5'd0) ? {12'h0, rd, 3'b000, 5'd1, 7'h67}             // c.jalr(rd=0はc.ebreak:未対応)
                                : {7'h00, rs2, rd, 3'b000, rd, 7'h33});        // c.add
      5'b10_110: w_ir = {i_swsp[11:5], rs2, 5'd2, 3'b010, i_swsp[4:0], 7'h23}; // c.swsp
      default: ;
    endcase
  end
endmodule
...
// 5段パイプライン 完全RV32I
// ============================================================
module m_proc_rvc(w_clk, r_rslt, r_done, uart_tx, uart_rx);
  input  wire w_clk;
  ...
  reg P1_cmp=0, P2_cmp=0;           // RVC: この命令は16bit圧縮だったか
  ...
  wire [31:0] w_br_truepc = (P2_b & P2_v & w_brcond) ? P2_tpc : (P2_pc + (P2_cmp ? 32'd2 : 32'd4));  // RVC: 圧縮分岐の次は+2
  ...
  wire [31:0] w_imem_word, w_imem_insn, w_imem_insn_hi, w_dmem_insn;
  m_am_imem m3 (r_pc, w_imem_insn, w_imem_insn_hi, P3_alu, w_imem_word);  // +次ワード(RVCまたぎ用)
  // ---- RVCフェッチ: 常に連続2ワード読み、pcから始まる32bitを組合せで組み立てる ----
  //   pc[1]=1 → {次ワード下位16, 現ワード上位16}(境界またぎもこれで解決、バッファ不要)。
  //   DMEMフェッチ(ブートローダ0x1_xxxx)は4B整列のrv32im限定(mkloadは非C)なのでpc[1]=0のみ。
  wire [31:0] w_fet = (r_pc[31:16]==16'h0001) ? w_dmem_insn : w_imem_insn;
  wire [31:0] w_raw = r_pc[1] ? {w_imem_insn_hi[15:0], w_imem_insn[31:16]} : w_fet;
  wire w_cmp = (w_raw[1:0] != 2'b11);                      // 下位2bit!=11 → 16bit圧縮命令
  wire [31:0] w_exp;
  m_rvc mrvc (w_raw[15:0], w_exp);                         // 16bit→32bit展開
  assign w_ir  = w_cmp ? w_exp : w_raw;                    // 以降のパイプは常に32bit命令を見る
  assign w_npc = r_pc + (w_cmp ? 32'd2 : 32'd4);           // 逐次PC: 圧縮なら+2
  ...
  always @(posedge w_clk) if (!w_stall) begin
    ...
    P2_m <= w_muldiv;
    {P1_cmp, P2_cmp} <= {w_cmp, P1_cmp};   // 圧縮命令フラグ(リンク値/次PCが+2になる)
  ...
  always @(posedge w_clk) begin
    ...
    P3_alu <= P2_csr ? w_csr_old : (w_jump ? (P2_pc + (P2_cmp ? 32'd2 : 32'd4)) : (P2_m ? w_m_out : w_alu));  // RVC: c.jal/c.jalrのリンクは+2
    P3_rd  <= P2_rd;
...
// ---- KV260 top: PSのpl_clk0 + m_proc_rvc + UART(PMOD) + VIO ----
module m_top_kv260(uart_tx, uart_rx);
  ...
  m_proc_rvc m1 (w_clk, w_rslt, w_done, uart_tx, uart_rx);
