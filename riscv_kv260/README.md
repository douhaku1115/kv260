...
## RV32C（圧縮命令）拡張 — 第5段-15

単一コア（console 系）に **RV32C＝16bit 圧縮命令**を追加した（`src/main_vio_rvc.v`、コア= `m_proc_rvc`。
console.v はそのまま、RVC はコピーで並行開発）。C 命令はすべて既存 32bit 命令の短縮形なので、
**デコード時に 32bit へ展開すればパイプライン本体は無変更**で済む。

- **展開器 `m_rvc`**（組合せ）: quadrant0/1/2 の RV32 サブセット約 25 命令
  （c.addi/c.lw/c.sw/c.mv/c.j/c.jal/c.beqz/c.li/c.lui/c.slli/c.sub…）を 16bit→32bit 変換。
  即値のスクランブル解きが最大の敷居（形式ごとにビット配置が違う）。
- **フェッチ**: 教科書的には「またぎ対応のフェッチバッファ」が要る所を、**常に連続 2 ワードを読む**
  方式で回避（IMEM に次ワード読みポートを追加）。`pc[1]=1` なら
  `{次ワード[15:0], 現ワード[31:16]}` を組合せで組み立て——**ワード境界をまたぐ 32bit 命令も
  状態機械なしで処理**できる。
- **PC 制御**: 逐次 PC は `+2/+4`（圧縮判定＝下位 2bit≠11）。圧縮フラグを P1→P2 に伝搬し、
  c.jal/c.jalr の**リンク値** と分岐の**真の次 PC** を `pc+2` に補正。
- **★分岐予測の落とし穴**: BTB のタグが `pc[31:8]` だけだと、2 バイト違いの PC が同じエントリに
  ヒットして**非分岐命令が誤リダイレクト→誤実行**する。**タグに `pc[1]` を追加**（正当性に必須）。
- ブートローダ（`load`, 0x13000）は 4B 整列の rv32im のまま（mkload は非 C）。
- gcc は `MARCH=rv32imc_zicsr` を `build_gcc.sh` に渡すだけ（既定は従来の rv32i_zicsr）。

効果（コードサイズ）と検証:

| プログラム | rv32i(m) | rv32imc | 削減 |
|---|---|---|---|
| Σ0..100 (main.c) | 17語 | **12語** | 29% |
| KOZOS シェル一式 | 2167語 | **1509語** | **30.4%** |

**KV260 実機で確認済み**（sum=5050 / calc 6*7=42 / ワーカー3本が cnt=78万超まで並行カウント）。
sim で ①非圧縮の既存シェルが素通しで動作（回帰）②圧縮 Σ＝0x13BA ③**圧縮版フル KOZOS シェル**で
sum/run(ワーカー+プリエンプション)/calc(乗算)/ps/load(ブートローダ fib=610) すべて動作。
`create_rvc.tcl` でビルド（IMEM は `asm_rvc.txt` を焼く）。WNS +0.589ns。
