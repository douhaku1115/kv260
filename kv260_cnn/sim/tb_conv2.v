// -----------------------------------------------------------------------------
// tb_conv2.v -- conv3x3.v を conv2 の設定（8ch 13x13 → 16ch 11x11）で検証する
//
//   conv1 と違うのは入力チャネルが 8 あること。ここで初めて
//   ci のループとアドレス計算 ci*ISIZE*ISIZE が試される。
//   入力は pool1 の出力（sim/test_pool1_out.hex）、期待値は sim/test_conv2_out.hex。
//
//   使い方: bash sim/run_sim.sh conv2
// -----------------------------------------------------------------------------
`timescale 1ns / 1ps
...
module tb_conv2;
    ...
    localparam integer ISIZE = 13;
    localparam integer OSIZE = ISIZE - 2;              // 11
    localparam integer SHIFT = 10;                     // CONV2_SHIFT
    localparam integer NIMG  = 10;
    localparam integer XLEN  = CIN  * ISIZE * ISIZE;   // 1352
    localparam integer YLEN  = COUT * OSIZE * OSIZE;   // 1936
    ...
    initial begin
        $readmemh("test_pool1_out.hex", source);
        $readmemh("test_conv2_out.hex", expected);
        ...
        $display("");
        if (total_bad == 0)
            $display("=== tb_conv2: 合格（%0d 枚すべて 1 バイトも違わない）===", NIMG);
        else
            $display("=== tb_conv2: 失敗（不一致 %0d 語）===", total_bad);
        $finish;
    end

    initial begin
        #20_000_000;
        $display("=== tb_conv2: 時間切れ ===");
        $finish;
    end
endmodule
