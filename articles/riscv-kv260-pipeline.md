...
教科書のコードがそのままでは動かない部分が多く、KV260特有の知識が必要ですが、一度環境を整えれば他の章のプロセッサも同様の手順で動かせます。本記事では続けて **5段版 m_proc9** と **7章の分岐予測（BTB+bimodal / gshare）** まで動かしました。分岐予測のように結果が変わらない機能は、VIOに **ミス予測回数** などの観測値を足し、教科書のHALTを真の停止に変えてカウンタを固定するのがポイントです。キャッシュ版 m_proc8_c2 等も同じ枠組みで移植できます。
