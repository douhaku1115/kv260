...
直接マップキャッシュの弱点は**コンフリクトミス**です。同じインデックスに当たる2つのアドレス（タグ違い）を交互に使うと、毎回互いを追い出し合って（スラッシング）ヒットしません。これを **2-wayセットアソシアティブ** `m_cache3`（code8-12）で解決します。直接マップを2つ並べ、**LRU（1bit）** で追い出す way を選ぶだけです。

効果が見えるよう、教科書のコンフリクト用プログラム（`asm_conflict.txt`、code8-11）を使います。これは低位 `0x0c` と高位 `0x8c`（インデックス同じ・タグ違い）を交互に実行するので、直接マップでは毎周ミスします。
...
直接マップは 1223 回もミスして 7938 サイクル。2-wayは両アドレスが別 way に共存でき、ミスはコールドスタートの **21 回**だけ、**約4倍高速**になりました。KV260実機VIOで `w_cyc=1928` / `w_miss=21` / 結果 `5050` を確認しました（VIOのプローブ名は接続したワイヤ名 `w_rslt`/`w_cyc`/`w_miss` で、内部レジスタ名では出ない点に注意）。

### 2ワードブロックにする（空間的局所性）

もう一つのキャッシュの軸が**ブロック化**です。1ライン＝2ワードの `m_cache2`（code8-10）にすると、ミス時に隣の命令も一緒に載るので（**空間的局所性**）、主記憶への転送回数が約半分になります。

ただし教科書の `m_cache2` は**単体モジュールしか無く**、プロセッサ統合は載っていません。そこでブロック充填に対応した `m_proc8_c2b` と、ミス時に2ワードを一括で返すバースト主記憶 `m_imem_blk` を自作して繋ぎました。効果が見えるよう、ループではなく**逐次プログラム**（`addi` を50個並べたもの）を使います。
...
逐次コードでは1回の転送が2命令を運ぶので、転送回数が 54→27（半分）、サイクルも **約1.7倍高速**になりました。KV260実機VIOで `w_cyc=189` / `w_miss=27` / 結果 `50` を確認。なお、Σのようなループ主体のコードだと転送は半減してもサイクル差は小さく、ブロックの効果は**コードの逐次性に依存**します。

これで8章のキャッシュは **直接マップ → 2-wayセットアソシアティブ → 2ワードブロック** の3種が揃いました。
...
教科書のコードがそのままでは動かない部分が多く、KV260特有の知識が必要ですが、一度環境を整えれば他の章のプロセッサも同様の手順で動かせます。本記事では続けて **5段版 m_proc9**、**7章の分岐予測（BTB+bimodal / gshare）**、**8章の命令キャッシュ（直接マップ／2-wayセットアソシアティブ／2ワードブロック）** まで動かしました。分岐予測やキャッシュのように結果（5050）が変わらない機能は、VIOに **ミス予測回数** や **サイクル数** などの観測値を足し、教科書のHALTを真の停止に変えてカウンタを固定するのがポイントです。
