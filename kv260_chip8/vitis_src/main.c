...
#define CYCLES_PER_FRAME 15            /* 1 フレームの命令実行数 (↓でゲーム全体が遅くなる) */
#define PHOSPHOR_FRAMES  2             /* 残光: 消灯後この数フレーム表示維持 (ちらつき低減) */

/* CHIP-8 quirk (ROM ごとに挙動が変わる。Invaders で不具合が出たら切替) */
...
/* Space Invaders [David Winter] — 1301 bytes
 *   key 4=左, 6=右, 5=発射  (キーボード q=左, e=右, w=発射) */
...
static void chip8_cycle(void)
{
    ...
    case 0x8000:
        ...
        case 0x5:
            ...
            break;
        case 0x6: {
            uint8_t s = QUIRK_SHIFT_VY ? V[y] : V[x];
            V[x] = s >> 1;
            V[0xF] = s & 0x1;
        } break;
        case 0x7:
    ...
    case 0xF000:
        ...
        case 0x33:                              /* BCD */
            ...
            break;
        case 0x55:
            for (int k = 0; k <= x; k++) mem[(I + k) & 0xFFF] = V[k];
            if (QUIRK_LOADSTORE_I) I += x + 1;
            break;
        case 0x65:
            for (int k = 0; k <= x; k++) V[k] = mem[(I + k) & 0xFFF];
            if (QUIRK_LOADSTORE_I) I += x + 1;
            break;
        default: break;
        }
...
int main(void)
{
    ...
    xil_printf("\r\n==== CHIP-8 Phase F: Space Invaders ====\r\n");
    xil_printf("q=left  e=right  w=fire   Enter=restart\r\n");
