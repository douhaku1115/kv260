...
- Step 1〜9: 実機動作確認済み、origin/main の kv260_mips/ に push 済み
- クロック: 20MHz（32bit 組み合わせ除算器のタイミング対策、rebuild.tcl で設定）

## 今後のロードマップ

- Step 10: C言語実行（mips-gccクロスコンパイル）
- Step 11: 例外処理（CP0レジスタ、syscall、オーバーフロー）
...
- Vitis WS: `E:\Xilinx\project_vitis\kv_mips10`（kv_mips2〜9 は古い）
